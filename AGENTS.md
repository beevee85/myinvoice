...
- `web/dist/` — produkční build frontendu; **NENÍ v gitu** (`.gitignore:16,19`, v historii nikdy nebyl). Vzniká až na cílovém stroji (u nás při buildu image; upstream ho nově staví i v CI do release bundlu).
...
### Frontend
- Po každé změně ve `web/src` spusť build — `dist/` je to, co se nasazuje a testuje; samotný `vue-tsc` nestačí. Výstup se ale **necommituje**, viz recept v Příkazech (upstream ho do release balíčku staví CI, `.github/workflows/docker-publish.yml`).
