...
final class DocumentTrashPolicy
{
    ...
    public function blockersForPurchaseInvoice(array $row): array
    {
        ...
        // 2. Vazby na jiné doklady a úhrady (nikdy nepřebít).
        $linked = [];
        if (!empty($row['advance_purchase_invoice_id'])) {
            $linked[] = 'doklad je propojen se zálohou';
        }
        $st = $pdo->prepare('SELECT COUNT(*) FROM purchase_invoices WHERE advance_purchase_invoice_id = ?');
        $st->execute([$id]);
        if ((int) $st->fetchColumn() > 0) {
            $linked[] = 'na doklad je navázané vyúčtování zálohy';
        }
